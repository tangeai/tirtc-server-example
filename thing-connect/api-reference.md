...
[`nginx.conf.example`](../thing-connect/nginx.conf.example)。
...
> 下列端点不再重复列出该通用错误。
>
> **跨域**: call-server 不加 CORS。H5 联系人页面通过 nginx 反向代理跟 user-server 统一到同一个域名下（见仓库根目录 [`nginx.conf.example`](../thing-connect/nginx.conf.example)：`/v1/call/*` 转发到 call-server，其余转发到 user-server），浏览器全程同源。
