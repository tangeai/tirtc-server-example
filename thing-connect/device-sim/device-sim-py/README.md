...
程序启动后进入实时流监听。每个 H5/App 使用独立连接，模拟器最多同时保留 4 个实时查看连接，共享一份媒体源，并把每帧发送给所有已订阅对应 stream 的连接。连接建立时，模拟器分别为该连接订阅下行音频 14 和下行视频 15，单路订阅失败不终止实时查看；只有收到该连接对上行音频 10 或视频 11 的订阅回调后，才向该连接发送对应媒体。紧随建连到达的订阅会由有界待激活记录保存，并在媒体任务就绪后继续生效；单端取消订阅或断开不影响其他观看端。`--up-video-file` 为空或设为 `audio-only` 时，只提供音频。
...
- H5 未订阅音频或视频时，设备不读取也不发送对应媒体；取消其中一种订阅只停止该媒体。
- 发送繁忙或关键帧恢复导致视频前进到下一个 IDR 时，音频文件同步到相同媒体位置，音画内容保持对应。
- 设备端持续输出实时流相关日志。
- 如果 H5 只订阅视频，设备只发送视频，不发送音频。
...
- 小程序侧能收到设备音频和视频。
- 设备侧日志可看到收到 `0x2000` 后订阅对端媒体；本端只接收订阅成功的 stream，并在对端订阅回调后发送对应音频或视频。
...
从“我的设备 → 多人对讲”安排此模拟器加入房间，或在终端使用 `room create [四位密码]`、`room join 六位房间号 [四位密码]`、`room leave`。`room status` 查看当前房间和成员；`room ptt down` 开始本机发言，`room ptt up` 停止发言。

默认仅显示房间状态、成员变化、发言操作和异常。排查问题时输入 `room debug on` 开启信令耗时与音频统计，输入 `room debug off` 关闭；诊断日志不包含原始音频或令牌。

本机麦克风对讲上行使用 20ms 音频帧，时间戳按音频时长连续递增。

加入后默认静音收听。`--with-mic` 模式使用本机麦克风、扬声器及现有 A-law 音频配置；文件模式只在 PTT 按下时发送上行文件，下行不保存录音。AI、设备通话和微信通话会暂停对讲，结束后自动同步并恢复期望房间。操作接口使用服务发现得到的 call-server 地址，设备重启和 MQTT 重连均会重新读取服务端关系。协议、容量和部署要求见 [设备多人对讲](../../device-room.md)。
