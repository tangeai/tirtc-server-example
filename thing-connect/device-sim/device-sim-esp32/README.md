...
octal PSRAM），使用 ESP-IDF 5.5.x 和 TiRTC SDK 2.3.0。默认 Demo 不依赖 TF 卡，
...
只需要设备上线、H5 音视频和 AI 对讲的开发者，使用
[ESP32-S3 工程模板](../ESP32_STARTER.md)生成独立的最小工程。本目录保留完整参考实现，适合查阅全部协议和会话处理。
...
```bash
idf.py -DTIRTC_SDK_DIR=/absolute/path/to/espressif-esp32s3/2.3.0 build
```
