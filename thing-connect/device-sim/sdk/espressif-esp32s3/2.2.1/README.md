...
默认 SDK 包关闭 trace，`StaticSemaphore_t = 84 bytes`。如果接入工程打开 trace，尺寸可能变为 `92 bytes`，预编译 SDK 和运行工程的对象布局会不一致，可能导致内存覆盖或线程异常。
...
工程必须使用其他 tick、trace 或 stats 配置时，需要重新获取匹配的 SDK 包，并提供：
