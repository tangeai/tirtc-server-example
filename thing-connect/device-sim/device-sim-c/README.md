...
- H5/App 实时流和设备互呼中的双向订阅门禁：本端订阅成功后才接收，对端订阅回调后才发送对应媒体。
...
每个 H5/App 使用独立连接。参考实现最多同时保留 4 个实时查看连接，共享一份媒体源，并把每帧发送给所有已订阅对应 stream 的连接。设备为每个连接分别订阅下行音频 14 和下行视频 15，单路订阅失败不终止实时查看；上行音频 10、视频 11 分别等到该连接的订阅回调后才向该连接发送。连接回调先登记有界待激活记录，紧随建连到达的订阅会在媒体任务就绪后继续生效；单端取消订阅或断开不影响其他观看端。设备互呼建立业务接通后，双方订阅对端音频 10，视频通话还订阅视频 11；接收回调同时校验当前连接、本端订阅状态和 stream。取消订阅、断开或会话代次变化会撤销对应权限。
