...
首次安装页面持续显示结构化失败原因和处理建议。MySQL 预检分别验证迁移账号连接和运行账号登录；运行账号失败时按页面检查密码、账号锁定、TLS、MySQL 实际识别的来源 Host 及目标库 DML 授权。Redis 失败时检查地址、认证、DB 编号、防火墙和监听地址。客户可见响应不包含 SQL、Redis、MQTT 客户端原始错误、内网连接串或凭据，详细原因只进入受保护的服务日志。
