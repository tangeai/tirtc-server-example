...
配置定义中的 `name`、`group`、`description`、`default`、`secret_paths` 和 `fields` 用于生成管理表单。`fields` 是后端注册表提供的单一字段事实源，每项包含字段路径 `path`、中文名称 `label`、控件类型 `kind`，并可包含 `description`、`options`、`secret`、`providers`、`required` 和 `min`。Admin Web 根据这些元数据生成输入控件和基础校验；只有明确使用自定义编辑器的复杂配置才维护独立页面。
...
该接口只允许服务间网络访问，响应包含 `value`、解密后的 `secrets` 和 `revision`，并设置 `Cache-Control: no-store`。它不属于公网 Admin API。
...
- 密钥配置的查询响应不会返回明文；内部读取接口仅对持有 `internal.key` 的服务开放。
