...
| 设备归属 | 一台设备同一时间只能加入一个房间；创建新房间或加入其他房间前，必须先退出当前房间 |
...
页面和 `/v1/call/group/*` 接口由 call-server 提供。统一域名部署时，网关按路径转发到 call-server。请求字段、鉴权和错误码见 [HTTP API](api-reference.md#多人对讲call-server)。
