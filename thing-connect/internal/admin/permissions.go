...
var PermissionDefinitions = []PermissionDefinition{
	...
	{Code: "board.read", Name: "查看开发板目录", Group: "内容管理", Description: "查看官网开发板资料和上下架状态"},
	{Code: "board.write", Name: "管理开发板目录", Group: "内容管理", Description: "新增、编辑、复制、排序及上下架开发板资料"},
...
var defaultMenus = []menuSeed{
	...
	{Code: "boards", Name: "开发板管理", Path: "/boards", Permission: "board.read", Sort: 30, ParentCode: "business", MenuType: 2},
...
var defaultRoles = []roleSeed{
	{Code: "super_admin", Name: "超级管理员", Sort: 1, Remark: "拥有全部管理权限；仅授予系统负责人", Permissions: AllPermissions},
	{
		...
		Permissions: []string{"dashboard.read", "service.status.read", "user.read", "user.status.write", "user.quota.write", "user.password_reset", "device.read", "device.unbind", "device.import", "board.read", "board.write", "job.read", "job.retry", "audit.read", "login_log.read"},
		Menus:       []string{"overview", "users", "devices", "boards", "jobs", "login-logs", "audit-logs"},
	},
	{
		Code: "technical_support", Name: "技术支持", Sort: 20, Remark: "只读查询用户、设备、绑定历史和任务状态",
		Permissions: []string{"dashboard.read", "service.status.read", "user.read", "device.read", "board.read", "job.read"},
		Menus:       []string{"overview", "users", "devices", "boards", "jobs"},
	},
	{
