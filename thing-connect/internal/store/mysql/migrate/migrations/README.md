...
`001_*.sql` 文件。执行器按文件名排序后统一执行，只写入一条组件版本记录。
拆分文件是为了区分职责，不代表安装时存在多次升级。

文件通过 `go:embed` 打包进二进制。执行器启动时自动发现
`core/NNN_*.sql` 和 `admin/NNN_*.sql`，校验每个组件的版本从 1 开始且保持连续，
再按版本和文件名逐条执行，并据此计算当前版本。

首次安装和 `deploy-prod.sh update` 使用同一套迁移，无需在 Go 代码或安装脚本中
重复登记版本数字和文件路径。
