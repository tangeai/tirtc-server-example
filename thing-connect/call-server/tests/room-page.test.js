...
async function page(respond = async () => ({code: 200, data: {desired_state: 'joined', state: 'waiting_device', room_code: '001234'}}), readCurrent = null) {
  const nodes = new Map();
  const node = id => {
...
test('HTTP create sends a request and displays the assigned room', async () => {
  ...
  assert.equal(p.posts[0].url, '/v1/call/group/web/device/test-device/create');
  assert.deepEqual(JSON.parse(JSON.stringify(p.posts[0].headers)), { 'Content-Type': 'application/json', Authorization: 'Bearer test-token' });
...
test('network failure is visible and a later click can retry', async () => {
  let attempts = 0;
  const p = await page(async () => {
...
test('business rejection preserves entered password and shows the reason', async () => {
  const p = await page(() => ({code: 40320, msg: '房间密码错误，请重新输入'}));
...
test('joining reads authoritative room counts instead of command defaults without polling', async () => {
  const p = await page(async () => ({code:200,data:{desired_state:'joined',state:'assigned',room_code:'668740',online:false,online_count:0}}), async posts => posts ? {desired_state:'joined',state:'joined',room_code:'668740',online:true,online_count:2} : {desired_state:'left'});
...
test('successful command with failed status read preserves success and never invents zero participants', async () => {
  const p = await page(async () => ({code:200,data:{desired_state:'joined',state:'assigned',room_code:'668740',online:false,online_count:0}}), async posts => {if(posts)throw Error('network');return {desired_state:'left'};});
  await p.change('join',{room_code:'668740',password:''});
