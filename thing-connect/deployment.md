...
本文给出 Ubuntu 22.04/24.04 上可复制执行的首次安装流程，以及安装完成后必须进行的服务验收、业务配置、进程托管、HTTPS 和安全收尾。默认安装目录为 `/opt/thing-connect`。安装完成后的软件更新、数据库升级和各团队自有发布流程不在本文范围内。
