...
[![C](https://img.shields.io/badge/Device%20Simulator-C-A8B9CC?logo=c&logoColor=black)](thing-connect/device-sim/device-sim-c/README.md)
...
> 未看到验证码，或提示 SDK、依赖、素材文件错误时，先查看 [device-sim-py README](thing-connect/device-sim/device-sim-py/README.md) 的环境搭建与失败排查。验证码过期或绑定超时后，停止程序并重新执行本步骤即可。
...
这一步用本地文件代替真实采集，无需接摄像头、麦克风或扬声器。在目标 Linux 设备上编译 「C 参考实现」和匹配的 TiRTC C SDK（也可交叉编译），继续从本地文件读写音视频；当 H5 能看到该设备的画面、听到声音时，就验证了目标设备的网络、SDK 集成、设备上线、MQTT 与实时音视频收发均可用。
...
4. 在 H5 设备列表打开该设备，确认能出图、出声；至此完成真实 Linux 设备的首个闭环。
...
| 2 | 在真实设备上逐项集成 | 按“音视频推流 → AI 对讲 → VoIP 对讲 → 设备互呼”逐项实现。可参照 [「C 参考实现」代码](thing-connect/device-sim/device-sim-c/README.md) 中已验证的上线、MQTT、TiRTC 和媒体收发流程，并按 [thing-connect 开发者文档](thing-connect/README.md) 的四项能力验收。 |
...
> Python 与 「C 参考实现」使用相同的设备上线和业务协议；前面已经跑通的流程可直接复用到真实设备。
